<template>
  <div class="bg-[#111016] w-full h-dvh">
    <div dir="rtl" class="h-[15%] flex justify-between px-8 items-center">
      <div class="flex flex-row-reverse gap-4 items-center">
        <Menu as="div" class="relative inline-block text-left">
          <MenuButton class="bg-[#486BE717] z-50 relative px-4 py-3 rounded-lg">
          ...
          >
            ...
            >
              ...
              <MenuItem>
                <button @click="go_profile_page" class="text-left">
                  Profile
                </button>
              </MenuItem>
              <MenuItem>
    ...
    <div class="h-[85%] flex flex-col items-center">
      <TabGroup>
        <TabList
          class="flex justify-around h-[8%] w-[90%] border-b border-[#1B1620]"
        >
          <Tab
        ...
        <TabPanels class="h-[92%] w-full overflow-auto">
          <TabPanel class="h-full overflow-auto pt-5">
            ...
            >
              <img
                src="../../public/images/empty-chat.png"
                class="w-[50%]"
                alt=""
              />
...
const fetch_conversations = () => {
  axiosConfig
    .get("conversations")
    .then((res) => {
      conversationsList.value = res.data.data.filter(
        (item) =>
          item.conversationId.includes(authStore.userLoginInfo.uid) &&
          item.conversationWith?.uid !== "app_system" &&
          item.conversationWith.uid !== authStore.userLoginInfo.uid
      );
    })
    .catch((error) => {
